...
srand(time(0));
int Input1, Input2, Input3, NumberOfInputs, TotalInputs, Output1, Output2;
int P = 2;
double Total;
...
for ( P; P <= 100; P = P + 2){
    ...
    for (int i = 0; i < Iterations; i++){
        ...
        Input2 = RBG(P);
        Input3 = RBG(P);
        NumberOfInputs = Input1 + Input2 + Input3;
    ...
    Data << TotalInputs << "\t" <<Total << "\t" << Throughput << endl; // THIS LINE EXPORTS THE DATA TO A .TXT FILE
}
